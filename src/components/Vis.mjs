...
export class Vis extends Component {
  ...
  updateEdge(){
    this.state.cy.getElementById(l2.getName()).addClass('highlightEdge')
  }
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <button onClick={()=>{this.setState({cy: this.defineCy() })}} >Draw Graph</button>
          <button onClick={()=>{this.updateEdge()}} >tick</button>
        </div>
